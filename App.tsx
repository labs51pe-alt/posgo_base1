...
const App: React.FC = () => {
  ...
  const handleCheckout = async (method: any, payments: any[]) => {
      ...
      try {
          ...
          const msg = error?.message || (typeof error === 'string' ? error : JSON.stringify(error)) || "Error desconocido";
        ...
        <Layout currentView={view || ViewState.POS} onChangeView={setView} settings={settings} user={user} onLogout={handleLogout}>
            ...
            {view === ViewState.PURCHASES && (
              <PurchasesView 
                ...
                purchases={purchases} 
                settings={settings} 
                onProcessPurchase={async (pur, updated) => { 
                  try { 
                    ...
                    alert("ERROR AL REGISTRAR COMPRA: " + msg + "\n\nIntenta ir a Super Admin > REPARAR DB (SQL) si el error persiste.");
                    throw e; 
                  } 
                }} 
