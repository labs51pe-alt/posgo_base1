                ...
                <div className="flex-1 flex flex-col overflow-hidden p-8 gap-6 animate-fade-in">
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm shrink-0 items-end">
                        <div className="lg:col-span-2 space-y-3">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Elegir Proveedor</label>
                        ...
                        <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">N° Documento</label>
                    ...
                    <div className="flex-1 flex flex-col lg:flex-row gap-6 overflow-hidden min-h-0">
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
                                {filteredSearchProducts.map(p => (
                                    <button key={p.id} onClick={() => handleAddToPurchase(p)} className="w-full text-left p-4 rounded-2xl hover:bg-amber-50 flex items-center gap-4 transition-all border border-transparent hover:border-amber-100 mb-1">
                                        ...
                                        <div className="flex-1 min-w-0"><p className="font-bold text-slate-800 text-sm truncate">{p.name}</p><p className="text-[10px] font-bold text-slate-400">Stock: {p.stock}</p></div>
                        ...
                        <div className="flex-1 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead className="bg-slate-50/80 text-[10px] font-black uppercase text-slate-400 sticky top-0 z-10">
                                        <tr>
                                            ...
                                            <th className="p-6 text-right w-36">Costo U.</th>
                                            <th className="p-6 text-right w-36">P. Venta</th>
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {purchaseCart.map((item) => (
                                            <tr key={item.id}>
                                                ...
                                                <td className="p-3 w-28"><input type="number" className="w-full bg-slate-50 rounded-xl px-1 py-3 text-center font-bold" value={item.quantity} onChange={e => updatePurchaseItem(item.id, 'quantity', parseFloat(e.target.value) || 0)}/></td>
                                                <td className="p-3 w-36"><input type="number" className="w-full bg-white border border-slate-100 rounded-xl px-2 py-3 text-right font-bold" value={item.cost} onChange={e => updatePurchaseItem(item.id, 'cost', parseFloat(e.target.value) || 0)}/></td>
                                                <td className="p-3 w-36"><input type="number" className="w-full bg-white border border-slate-100 rounded-xl px-2 py-3 text-right font-bold text-indigo-600" value={item.newPrice} onChange={e => updatePurchaseItem(item.id, 'newPrice', parseFloat(e.target.value) || 0)}/></td>
                                                <td className="p-6 text-right font-bold">{settings.currency}{((item.cost || 0) * (item.quantity || 0)).toFixed(2)}</td>
                            ...
                            <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-end">
                                <button onClick={handleFinishPurchase} disabled={purchaseCart.length === 0 || !selectedSupplierId || isUpdating} className="px-10 py-5 bg-emerald-500 hover:bg-emerald-600 disabled:opacity-20 text-white rounded-[1.8rem] font-black shadow-xl flex items-center gap-3">
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {purchases.length === 0 ? (
                                    <tr><td colSpan={7} className="p-20 text-center text-slate-400 font-bold">No hay compras registradas</td></tr>
                                ) : purchases.map(p => (
                                    <tr key={p.id} onClick={() => setSelectedPurchase(p)} className="hover:bg-slate-50 cursor-pointer group">
                                        ...
                                        <td className="p-6"><span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${p.status === 'PAID' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>{p.status === 'PAID' ? 'Pagado' : 'Pendiente'}</span></td>
                                        <td className="p-6"><span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${p.received === 'YES' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'}`}>{p.received === 'YES' ? 'En Stock' : 'Pendiente'}</span></td>
            ...
            {activeTab === 'REPORTS' && (
                <div className="flex-1 overflow-y-auto p-8 animate-fade-in space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-rose-100 shadow-sm shadow-rose-50"><p className="text-[10px] font-black text-rose-400 uppercase tracking-widest mb-3">Deuda Pendiente</p><div className="flex items-center gap-4"><div className="w-12 h-12 rounded-2xl bg-rose-50 flex items-center justify-center text-rose-600"><CreditCard className="w-6 h-6"/></div><p className="text-4xl font-black text-rose-800">{settings.currency}{(reports.totalUnpaid || 0).toLocaleString()}</p></div></div>
            ...
            {activeTab === 'SUPPLIERS' && (
                <div className="flex-1 overflow-y-auto p-8 animate-fade-in">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        <button onClick={() => setIsSupplierModalOpen(true)} className="aspect-square bg-white border-2 border-dashed border-slate-200 rounded-[3rem] flex flex-col items-center justify-center gap-4 text-slate-400 hover:border-emerald-500 hover:text-emerald-600 transition-all">
                            <Plus className="w-14 h-14" /><span className="font-black text-[11px] uppercase tracking-widest">Nuevo Proveedor</span>
                        </button>
                        {suppliers.map(s => (
                ...
                <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[150] flex justify-center items-center p-4">
                    <div className="w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl animate-fade-in-up flex flex-col overflow-hidden border border-white/20">
                        <div className="p-8 border-b border-slate-50 bg-[#0f172a] text-white flex justify-between items-center">
                            <div><h3 className="text-2xl font-black tracking-tight mb-1">Detalle de Compra</h3><p className="text-slate-400 text-[10px] font-black uppercase">N° {selectedPurchase.invoiceNumber || 'S/N'}</p></div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 text-center">
                                    ...
                                    <button onClick={handleConfirmWarehouseEntry} disabled={selectedPurchase.received === 'YES' || isUpdating} className={`w-full py-3 rounded-xl text-[10px] font-black uppercase ${selectedPurchase.received === 'YES' ? 'bg-indigo-600 text-white' : 'bg-amber-500 text-white'}`}>{selectedPurchase.received === 'YES' ? 'INGRESADA' : 'RECIBIR AHORA'}</button>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><Inbox className="w-4 h-4 text-indigo-500"/> PRODUCTOS</h4>
